...
FROM Facilities
WHERE membercost < (.20 * monthlymaintenance) AND membercost > 0
...
SELECT * 
FROM (SELECT DISTINCT f.name as facility_name, 
       	CASE WHEN m.memid = 0 THEN 'Guest'
       	ELSE concat(m.surname,' ',m.firstname) END AS name,
  	   	CASE WHEN m.memid = 0 THEN f.guestcost
       	ELSE f.membercost END AS cost
...
SELECT f.name AS facility_name, SUM(CASE WHEN m.memid = 0 THEN f.guestcost
